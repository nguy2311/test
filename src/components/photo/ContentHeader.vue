<script setup lang="ts">
import SearchComponent from "@/components/searchComponent/SearchComponent.vue";
import { Button } from "@/components/ui/button";
import { Topic } from "@/model/topic";
import { defineProps } from "vue";
import CollectionItems from "./ColectionItems.vue";
import TopContributors from "./TopContributors.vue";
import { useTypeStore } from "@/store/typeStore";
const typeStore = useTypeStore();

const props = defineProps<{
  topic: Topic | null;
}>();
</script>

<template>
  <div
    class="flex max-w-[1336px] gap-[24px] px-[20px] pt-[56px] box-border mx-auto items-end"
  >
    <div v-if="typeStore.type === 'photo'" class="w-3/4">
      <h1 class="text-4xl font-bold mb-2">Unsplash</h1>
      <p class="text-gray-600">The internet’s source for visuals</p>
      <p class="text-gray-600">Powered by creators everywhere.</p>
      <div class="mt-4">
        <SearchComponent />
      </div>
    </div>

    <div
      v-else-if="props.topic !== null"
      class="w-3/4 flex flex-col gap-[24px]"
    >
      <h1 class="text-4xl font-bold mb-2">{{ props.topic.title }}</h1>
      <p class="text-gray-600">{{ props.topic.description }}</p>
      <Button class="w-fit text-[15px] h-[42px] leading-[40px]">
        <p>Submit to</p>
        <p class="font-bold">{{ props.topic.title }}</p>
      </Button>
    </div>

    <div
      v-if="typeStore.type === 'photo'"
      class="w-1/4 border border-solid border-[#d1d1d1] rounded-lg"
    >
      <div class="p-[20px]">
        <h2 class="text-lg font-semibold mb-4">Collections</h2>
        <CollectionItems />
        <a
          href="#"
          class="text-sm text-[#767676] mt-4 inline-block hover:text-black"
          >See all</a
        >
      </div>
    </div>

    <div
      v-else-if="props.topic !== null"
      class="w-1/4 border border-solid border-[#d1d1d1] rounded-lg"
    >
      <div class="p-[20px]">
        <h2 class="text-lg font-semibold mb-4">Top contributors</h2>
        <TopContributors :topic="props.topic" />
        <a
          href="#"
          class="text-sm text-[#767676] mt-4 inline-block hover:text-black"
          >See all</a
        >
      </div>
    </div>
  </div>
</template>
